...
class WorkoutManager {
    ...
    init() {
        ...
        load()
        print("🔵 Datos cargados. Rutinas count: \(routines.count)")
        if routines.isEmpty {
            print("🔵 Rutinas vacías, cargando DefaultData...")
            routines = DefaultData.all
            print("🔵 DefaultData cargado. Rutinas count: \(routines.count)")
            saveRoutines()
        } else {
            print("🔵 Migrando descripciones...")
            migrateDescriptions()
        }
        print("🔵 WorkoutManager init completado")
    }
